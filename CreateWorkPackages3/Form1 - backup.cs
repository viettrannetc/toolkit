...
namespace CreateWorkPackages3
{
	public partial class Form1 : Form
	{
		...
		//private List<ListViewItem> _rootListViewDaily = new List<ListViewItem>();
		...
		{
			//Daily_log_textbox.Clear();
			//Daily_log_textbox.ResetBackColor();
			...
			var item1 = new ToolkitWPModel()
			{
				...
				Release = 13,//TODO: release of the feature and US
		...
		private void GetDailyData_Click(object sender, EventArgs e)
		{
			//lsvDaily.Items.Clear();
		...
		private void BuildProgressTracking(List<WPItemModel> dataSource)
		{
			...
			_wpItemsLocal.SortPriority();
			_wpItemsLocal = _wpItemsLocal
			...
			table.Columns.Add(_column_Status, typeof(string));          //2	TODO: consider adding color yellow/red
			...
			table.Columns.Add(_column_Assignee, typeof(string));        //5
																		//table.Columns.Add("Due Date"));	- TODO: auto calculated by estimate
				...
				else
				{
					...
					if (dueDate.Date >= DateTime.UtcNow.Date)
					{
						...
						if (remainingHoursReality - remainingHoursEstimation < -8)
						{
							...
							reason = $"Remaining ({remainingHoursReality}) > reality ({remainingHoursReality})";
						}
					}
					else   //Overdude - RED					
					{
		...
		private void tabDetailsPlan_GridView_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			if (e.ColumnIndex >= 0)
			{
				...
				////update startDate
				////if (e.ColumnIndex == 3)
				//if (tabDetailsPlan_GridView.Columns[e.ColumnIndex].Name.Contains(_column_Start))
				//{
				//	var wpId = int.Parse(tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-')[tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-').Count() - 1].Trim());
				//	var currentRowData = _wpItemsLocal.First(d => d.WPId == wpId);

				//	var cellOldData = currentRowData.WPStart.HasValue
				//		? currentRowData.WPStart.Value.Date.ToShortDateString()
				//		: string.Empty;
				//	var cellNewData = tabDetailsPlan_GridView[e.ColumnIndex, e.RowIndex].Value.ToString();

				//	if (cellOldData == cellNewData) return;

				//	var existingChange = _wpItemsLocalChanges.FirstOrDefault(d => d.WPId == wpId && d.Property == _column_Start);
				...
				//	var cellOldData = currentRowData.WPAssignee;
				//	var cellNewData = tabDetailsPlan_GridView[e.ColumnIndex, e.RowIndex].Value.ToString();

				//	if (cellOldData == cellNewData) return;

				//	var existingChange = _wpItemsLocalChanges.FirstOrDefault(d => d.WPId == wpId && d.Property == _column_Assignee);
				//	if (existingChange == null)
				//	{
				//		_wpItemsLocalChanges.Add(new WPItemChangesModel
				//		{
				...
				////update Estimate
				////if (e.ColumnIndex == 5)
				//if (tabDetailsPlan_GridView.Columns[e.ColumnIndex].Name.Contains(_column_Estimate))
				//{
				//	var wpId = int.Parse(tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-')[tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-').Count() - 1].Trim());
				//	var currentRowData = _wpItemsLocal.First(d => d.WPId == wpId);

				//	var cellOldData = currentRowData.WPEstimate;
				//	var cellNewData = tabDetailsPlan_GridView[e.ColumnIndex, e.RowIndex].Value.ToString();

				//	if (cellOldData == cellNewData) return;

				//	var existingChange = _wpItemsLocalChanges.FirstOrDefault(d => d.WPId == wpId && d.Property == _column_Estimate);
				//	if (existingChange == null)
				//	{
				//		_wpItemsLocalChanges.Add(new WPItemChangesModel
				//		{
				...
				////update Remaining
				////if (e.ColumnIndex == 6)
				//if (tabDetailsPlan_GridView.Columns[e.ColumnIndex].Name.Contains(_column_Remaining))
				//{
				//	var wpId = int.Parse(tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-')[tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-').Count() - 1].Trim());
				//	var currentRowData = _wpItemsLocal.First(d => d.WPId == wpId);

				//	var cellOldData = currentRowData.WPRemainingHour;
				//	var cellNewData = tabDetailsPlan_GridView[e.ColumnIndex, e.RowIndex].Value.ToString();

				//	if (cellOldData == cellNewData) return;

				//	var existingChange = _wpItemsLocalChanges.FirstOrDefault(d => d.WPId == wpId && d.Property == _column_Remaining);
				//	if (existingChange == null)
				//	{
				//		_wpItemsLocalChanges.Add(new WPItemChangesModel
				//		{
				...
				////update Depend On
				////if (e.ColumnIndex == 7)
				//if (tabDetailsPlan_GridView.Columns[e.ColumnIndex].Name.Contains(_column_DependOn))
				//{
				//	var wpId = int.Parse(tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-')[tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-').Count() - 1].Trim());
				//	var currentRowData = _wpItemsLocal.First(d => d.WPId == wpId);

				//	var cellOldData = currentRowData.WPDependOn;
				//	var cellNewData = tabDetailsPlan_GridView[e.ColumnIndex, e.RowIndex].Value.ToString();

				//	if (cellOldData == cellNewData) return;

				//	var existingChange = _wpItemsLocalChanges.FirstOrDefault(d => d.WPId == wpId && d.Property == _column_DependOn);
				//	if (existingChange == null)
				//	{
				//		_wpItemsLocalChanges.Add(new WPItemChangesModel
				//		{
				...
				////update due date
				////if (e.ColumnIndex == 9)
				//if (tabDetailsPlan_GridView.Columns[e.ColumnIndex].Name.Contains(_column_DueDate))
				//{
				//	var wpId = int.Parse(tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-')[tabDetailsPlan_GridView[0, e.RowIndex].Value.ToString().Split('-').Count() - 1].Trim());
				//	var currentRowData = _wpItemsLocal.First(d => d.WPId == wpId);

				//	var cellOldData = currentRowData.WPDueDate.HasValue
				...
				//	var existingChange = _wpItemsLocalChanges.FirstOrDefault(d => d.WPId == wpId && d.Property == _column_DueDate);
				//	if (existingChange == null)
				//	{
				//		_wpItemsLocalChanges.Add(new WPItemChangesModel
				//		{
				...
				//	var existingChange = _wpItemsLocalChanges.FirstOrDefault(d => d.WPId == wpId && d.Property == _column_Iteration);
				//	if (existingChange == null)
				//	{
				//		_wpItemsLocalChanges.Add(new WPItemChangesModel
				//		{
		...
		{
			//TODO: push changes to toolkit in a foreach by entity/id
			var toolkitWPItems = new List<ToolkitWPModel>();
			...
			foreach (var changes in changesByWP)
			{
				...
				toolkitWPItems.Add(new ToolkitWPModel()
				{
					...
					AssigneeId = string.IsNullOrWhiteSpace(data.WPAssignee)
						? 233 //TODO: how to clear assignee?
			...
			try
			{
				if (e.ColumnIndex > -1)
				{
					if (tabDetailsPlan_GridView.Columns[e.ColumnIndex].Name.Contains(_column_Assignee))
					{
						...
						l_objGridDropbox.DataSource = GetAssignee(); // Bind combobox with datasource.  
		...
		//	var witdh1stColumn = 200F;
		//	var witdhminumumColumn = 30F;
		...
		//	}
		//	//panel.RowStyles.Add(new RowStyle(SizeType.AutoSize, 20F));
		...
		//	{
		//		firstRow.ColumnStyles.Add(new ColumnStyle(SizeType.AutoSize) { });
		//	}
		//	firstRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = "Month" }, 0, 0);
		//	for (int i = 1; i <= durationInMonths.Count; i++)
		//	{
		//		firstRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = durationInMonths[i - 1].ToString() }, i, 0);
		//	}
		...
		//	}
		//	secondRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = "Week" }, 0, 0);
		//	for (int i = 1; i <= durationInWeeks.Count; i++)
		//	{
		//		secondRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = durationInWeeks[i - 1].ToString() }, i, 0);
		//	}
		//	panel.Controls.Add(secondRow, 0, 1);
		//	#endregion

		//	#region 3rd row - Iteration
		//	var durationInInteration = _iterationsPlanning.Select(i => i.Iteration).Distinct().ToList();
		//	var thirdRow = new TableLayoutPanel()
		//	{
		//		Dock = DockStyle.Fill,
		//		ColumnCount = durationInInteration.Count + 1,
		//		RowCount = 1,
		//	};
		//	for (int i = 0; i < thirdRow.ColumnCount; i++)
		//	{
		//		thirdRow.ColumnStyles.Add(new ColumnStyle(SizeType.AutoSize) {  });
		//	}
		//	thirdRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = "Iteration" }, 0, 0);
		//	for (int i = 1; i <= durationInInteration.Count; i++)
		//	{
		//		thirdRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = durationInInteration[i - 1].ToString() }, i, 0);
		//	}
		//	panel.Controls.Add(thirdRow, 0, 2);
		//	#endregion

		//	#region 4th row - Available / Workload / Allocated
		...
		//	{
		//		fourthRow.ColumnStyles.Add(new ColumnStyle(SizeType.AutoSize) {  });
		//	}
		//	fourthRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = "Available/Workload/Allocated" }, 0, 0);
		...
		//		fourthRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = $"{iteration.AvailableHours}/{iteration.Workload}/{iteration.AllocatedHours}" }, i, 0);
		//	}
		//	panel.Controls.Add(fourthRow, 0, 3);
		//	#endregion
		...
		//		{
		//			nextRow.ColumnStyles.Add(new ColumnStyle(SizeType.AutoSize) { });
		//		}
		//		nextRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = $"{feature.Title} - ({feature.Id})" }, 0, 0);
		...
		//			var actions = iteration.Items.First(f => f.Feature == $"{feature.Id} - {feature.Title}").Actions;

		//			nextRow.Controls.Add(new Label() { Dock = DockStyle.Fill, Text = $"{string.Join(" + ", actions)}" }, i, 0);
		//		}
		//		panel.Controls.Add(nextRow, 0, lastRow + 1);
		...
		///// <summary>
		///// Switch the tab - between Changes & Planning
		///// </summary>
		///// <param name="sender"></param>
		///// <param name="e"></param>
		//private void tabDetailsPlan_TabControl_SelectedIndexChanged(object sender, EventArgs e)
		//{
		//	//Click on Planning - Show Iteration column in Details GridView
		//	//Click on Changes - Hide Iteration column in Details GridView
		//}

		///// <summary>
		///// Click on each iteration and the detail data will be loaded to details areas
		///// </summary>
		///// <param name="sender"></param>
		///// <param name="e"></param>
		//private void tabDetailsPlan_Planning_Iteration_Click(object sender, EventArgs e)
		//{
		//	//Load data of the selected iteration to tabDetailsPlan_Planning_Iteration_Details_Label
		//	//Load data of the selected iteration to tabDetailsPlan_Planning_Iteration_Details_GridView
		//}
